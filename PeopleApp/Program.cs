...
namespace PeopleApp
{
    ...
    // Purpose: Problem Set 12 Question 7
    // Restrictions: None
    class PeopleApp
    {
        // Method: Main
        // Purpose: An application that holds record of students and teachers
                    ...
                    case "list":
                        ...
                        foreach (KeyValuePair<string, Person> thisEntry in people.sortedList)
                        {
                            ...
                            if (thisPerson.GetType() == typeof(Student))
                            {
                                ...
                                // goes through each course code string in student.courseCodes
                               foreach (string courseCode in student.courseCodes)
                                {
                                    // save Course that matches the course code as returnVal
                                    Course returnVal;

                                    // search each key-value pair for the key matching courseCode
                                    foreach (KeyValuePair<string, Course> myEntry in courses.sortedList)
                                    {
                                        // save the Course associated as returnVal
                                        if(myEntry.Key == courseCode)
                                        {

                                            returnVal = myEntry.Value;

                                            // create string to hold days of the week
                                            string returnString = "";

                                            // create a string containing each day of the week
                                            foreach (DayOfWeek dayOfWeek in returnVal.schedule.daysOfWeek)
                                            {
                                                returnString += (dayOfWeek.ToString() + " ");
                                            }
                                            
                                            // write the course information
                                            Console.Write(courseCode + " - " + returnVal.description + " " + returnString);
                                            Console.Write($"{returnVal.schedule.startTime:hh:mmtt}" + " - ");
                                            Console.WriteLine($"{returnVal.schedule.endTime:hh:mmtt}");
        ...
        // Method: EditPerson
        // Purpose: Add/Edit a student or teacher to the database
        public static void EditPerson(ref Person thisPerson)
        {
            ...
            if (thisPerson.GetType() == typeof(Student))
            {
                ...
                do
                {
                    // prompt the user to add course codes
                    Console.Write("Course code=> ");
                    string studentCode = Console.ReadLine();

                    // if the user entered something valid, add the course code to the courseCodes list
                    if (studentCode.Length != 0)
                    {
                        thisStudent.courseCodes.Add(studentCode);
                        bValid = false;
                    }
                    else { bValid = true; }
